...
import {
  buildDescriptionDocId,
  parseDescriptionDocId,
} from "@/components/chat/infra/blocksuite/descriptionDocId";
...
import {
  ...
  isDocRefDrag,
  setDocRefDragData,
} from "@/components/chat/utils/docRef";
...
export default function DocFolderForUser({
  ...
  const docs = useMemo(() => {
    if (!docsQuery.data?.success) return [] as SpaceUserDocResponse[];
  ...
  const docById = useMemo(() => {
    ...
    for (const d of docs) {
      if (typeof d?.docId === "number") map.set(d.docId, d);
    }
    return map;
  }, [docs]);

  const [localTagStore, setLocalTagStore] = useLocalStorage<
    Record<string, string[]>
  >(LOCAL_TAG_STORAGE_KEY, {});
  const localTagSpaceKey = useMemo(
    () => (spaceId > 0 ? String(spaceId) : ""),
    [spaceId],
  );

  const localTags = useMemo(() => {
    if (!localTagSpaceKey) return [] as string[];
    const rawList = localTagStore[localTagSpaceKey];
    if (!Array.isArray(rawList)) return [] as string[];
  ...
  const rememberLocalTag = useCallback(
    (tagPath: string) => {
      if (!localTagSpaceKey) return;
      const normalized = normalizeTagPath(tagPath);
      if (!normalized) return;
      setLocalTagStore((prev) => {
        const current = Array.isArray(prev[localTagSpaceKey])
          ? prev[localTagSpaceKey]
          : [];
        if (current.includes(normalized)) return prev;
        return {
          ...prev,
          [localTagSpaceKey]: [...current, normalized].sort((a, b) =>
  ...
  const folderNodes = useMemo(
    () => buildFolderNodes(docs, localTags),
    [docs, localTags],
  );
  ...
  const scheduleRenameFromEditor = useCallback(
    (docId: number, title: string) => {
      ...
      editorRenameTimerRef.current = window.setTimeout(() => {
        ...
        renameDocMutation.mutate(
          { spaceId, docId, title: finalTitle },
          {
  ...
  const [createDocTitleDefaultValue, setCreateDocTitleDefaultValue] =
    useState("");
  ...
  const [localTagCreateDefaultValue, setLocalTagCreateDefaultValue] =
    useState("");
  ...
  const submitCreateLocalTag = useCallback(() => {
    ...
    const normalized = normalizeTagPath(
      localTagCreateInputRef.current?.value ?? "",
    );
    if (!normalized) {
      toast.error("请输入标签路径");
      return;
  ...
  const requestUpdateTag = useCallback(
    (docId: number, currentTag: string | null | undefined) => {
  ...
  const [deleteDocConfirmId, setDeleteDocConfirmId] = useState<number | null>(
    null,
  );
  ...
  const confirmDeleteDoc = useCallback(async () => {
    ...
    try {
      const res = await deleteDocMutation.mutateAsync({
        spaceId,
        docId: deleteDocConfirmId,
      });
      if (!res?.success) {
  ...
  const [docCopyDropTagKey, setDocCopyDropTagKey] = useState<string | null>(
    null,
  );
  ...
  const handleSendDocToCurrentRoom = useCallback(
    async (doc: SpaceUserDocResponse) => {
      if (!onSendDocCard) {
        toast.error("当前页面不支持发送到群聊");
        return;
      }
      const docEntityId =
        typeof doc.docId === "number" && Number.isFinite(doc.docId)
          ? doc.docId
          : null;
  ...
  const handleDropDocRefToTag = useCallback(
    async (params: {
      tagKey: string;
      docRef: {
        docId: string;
      ...
      try {
        const tag =
          params.tagKey === UNTAGGED_KEY ? "" : normalizeTagPath(params.tagKey);
        if (tag.length > TAG_MAX_LENGTH) {
          toast.error(`标签长度不能超过 ${TAG_MAX_LENGTH}`);
          return;
        }
        ...
        const moveDocId =
          parsed?.entityType === "space_user_doc" &&
          parsed.docType === "description"
            ? parsed.entityId
            : null;
        const moveTargetDoc =
          moveDocId != null ? docById.get(moveDocId) : undefined;
        const pendingMode: "copy" | "move" =
          moveDocId != null && moveTargetDoc ? "move" : "copy";
          ...
          <div className="dropdown dropdown-end">
            ...
            >
              ...
              <li>
                <a
                  onClick={() => {
                    if (typeof doc.docId !== "number") return;
              ...
              <li>
                <a onClick={() => setDeleteDocConfirmId(doc.docId ?? null)}>
                  删除
                </a>
              </li>
            </ul>
  ...
  const renderFolderNode = (node: FolderNode) => {
    ...
    const pendingLabel =
      docCopyPendingState?.mode === "move" ? "移动中…" : "复制中…";
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div
        ...
        onDragOverCapture={(e) => {
          if (!spaceId || spaceId <= 0) return;
          if (!isDocRefDrag(e.dataTransfer)) return;
          e.preventDefault();
          ...
          const parsed = dragPayload
            ? parseDescriptionDocId(dragPayload.docId)
            : null;
          const isMoveInPlace =
            parsed?.entityType === "space_user_doc" &&
            parsed.docType === "description" &&
        ...
        onDropCapture={(e) => {
          ...
          const tagKey =
            tagEl?.getAttribute?.("data-tc-userdoc-tag") || UNTAGGED_KEY;
      ...
      <ToastWindow
        isOpen={localTagCreateOpen}
        onClose={() => setLocalTagCreateOpen(false)}
      >
        <div className="w-[min(520px,92vw)] p-6">
          ...
          <div className="text-xs opacity-60 mt-2">
            仅前端本地保存，拖拽文档到此文件夹时才会写入后端 tag。
          </div>
          <div className="flex justify-end gap-2 mt-4">
            <button
              type="button"
              className="btn btn-ghost"
              onClick={() => setLocalTagCreateOpen(false)}
            >
              取消
            </button>
            <button
      ...
      <ToastWindow
        isOpen={deleteDocConfirmId != null}
        ...
        <div className="w-[min(520px,92vw)] p-6">
          ...
          <div className="text-sm opacity-70">
            ...
            {deleteDocConfirmId != null
              ? (docById.get(deleteDocConfirmId)?.title ??
                `文档 #${deleteDocConfirmId}`)
              : ""}
          </div>
          <div className="text-xs opacity-60 mt-2">
            提示：仅软删除，可用于恢复/审计。
