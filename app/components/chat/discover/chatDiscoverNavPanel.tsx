...
interface ChatDiscoverNavPanelProps {
  ...
  activeArchiveTab?: "repositories" | "materialPackages" | "roles";
  activeSquareTab?: "repositories" | "materialPackages";
}
...
function parseRepositoryUpdateTitle(
...
function extractRepositoryId(response: any): number | null {
  ...
  if (isValidId(nested)) return nested;
...
export default function ChatDiscoverNavPanel({
  onCloseLeftDrawer,
  onToggleLeftDrawer,
  isLeftDrawerOpen,
  activeMode,
  ...
  const [expandedEntry, setExpandedEntry] = useState<DiscoverEntryKind | null>(
    null,
  );
  ...
  const repositoryFollowingQuery = useQuery({
    ...
    queryFn: () =>
      tuanchat.feedController.getFollowingMomentFeed({
        pageSize: REPOSITORY_UPDATES_PAGE_SIZE,
      }),
  ...
  useEffect(() => {
    ...
    if (!payload) {
      if (repositoryFollowingQuery.isError) {
        console.debug(
          "[discover-following-updates] query error",
          repositoryFollowingQuery.error,
        );
      }
      return;
    }
    ...
    if (Array.isArray(list)) {
      for (const item of list) {
        ...
        if (Number.isFinite(t)) types.set(t, (types.get(t) ?? 0) + 1);
      }
    }
    const first = Array.isArray(list) ? list[0] : null;
    const firstResponse = first?.response;
    const firstResponseKeys =
      firstResponse && typeof firstResponse === "object"
        ? Object.keys(firstResponse).slice(0, 12)
        : [];
    ...
    console.debug("[discover-following-updates] payload", {
      ...
      first: first
        ? {
            type: first?.type,
            responseKeys: firstResponseKeys,
            repositoryId: firstResponse?.repositoryId,
    ...
    repositoryFollowingQuery.error,
    repositoryFollowingQuery.isError,
    shouldLogDebug,
  ]);
  ...
  const repositoryUpdates = useMemo<FollowingUpdateItem[]>(() => {
    ...
    for (const item of list as FeedWithStatsResponse[]) {
      ...
      if (!repositoryId) continue; // 只显示能识别出仓库 ID 的动态

      const actorUserId = extractActorUserId(response);
      const timestamp =
        String(response?.createTime ?? response?.updateTime ?? "").trim() ||
        undefined;
      ...
      if (result.length >= MAX_UPDATES_TO_RENDER) break;
    }
  ...
  const openRepositoryDetail = useCallback(
    (repositoryId: number) => {
      if (!isValidId(repositoryId)) return;
  ...
  const renderUpdatesPanel = useCallback(
    (kind: DiscoverEntryKind) => {
      ...
      const isLoading = isRepository
        ? repositoryFollowingQuery.isLoading
        : false;
                ...
                <button
                  ...
                  onClick={() => {
                    if (u.kind === "repository") {
                      ...
                      handleNavigate("/chat/discover/material-packages");
                    }
                  }}
                >
  ...
  const repositoryToggleLabel =
    expandedEntry === "repository" ? "收起仓库关注更新" : "展开仓库关注更新";
  const materialToggleLabel =
    expandedEntry === "materialPackage"
      ? "收起素材包关注更新"
      : "展开素材包关注更新";
      ...
      <div className="px-3 pt-3 pb-2 space-y-4">
        <div>
          ...
          <div className="space-y-1">
            <div
              className={`${entryRowBase} ${activeMode === "square" && activeSquareTab === "repositories" ? entryRowActive : entryRowInactive}`}
            >
              <button
                type="button"
              ...
              >
                {repositoryUpdateCount > 0 && (
                  <span className="size-1.5 rounded-full bg-error" />
                )}
                {repositoryUpdateCount > 0 && (
                  <span>{repositoryUpdateCount}</span>
                )}
                <span
                  className={`ml-0.5 inline-block transition-transform ${expandedEntry === "repository" ? "rotate-90" : "rotate-180"}`}
                >
                  ▸
                </span>
              </button>
            ...
            <div
              className={`${entryRowBase} ${activeMode === "square" && activeSquareTab === "materialPackages" ? entryRowActive : entryRowInactive}`}
            >
              <button
                type="button"
                className="flex-1 min-w-0 text-left"
              ...
              >
                {materialPackageUpdateCount > 0 && (
                  <span className="size-1.5 rounded-full bg-error" />
                )}
                {materialPackageUpdateCount > 0 && (
                  <span>{materialPackageUpdateCount}</span>
                )}
                <span
                  className={`ml-0.5 inline-block transition-transform ${expandedEntry === "materialPackage" ? "rotate-90" : "rotate-180"}`}
                >
                  ▸
                </span>
              </button>
        ...
        <div>
          ...
          <div className="space-y-1">
            <div
              className={`${entryRowBase} ${activeMode === "my" && activeArchiveTab === "repositories" ? entryRowActive : entryRowInactive}`}
            >
              <button
                type="button"
                className="flex-1 min-w-0 text-left"
                onClick={() => handleNavigate("/chat/discover/my")}
                aria-label="进入仓库归档"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className="size-2 rounded-full bg-info opacity-80" />
                  <div className="truncate">仓库归档</div>
                </div>
              </button>
              <div aria-hidden="true" className="shrink-0 h-7 w-10" />
            </div>

            <div
              className={`${entryRowBase} ${activeMode === "my" && activeArchiveTab === "materialPackages" ? entryRowActive : entryRowInactive}`}
            >
              <button
                type="button"
                className="flex-1 min-w-0 text-left"
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className="size-2 rounded-full bg-success opacity-80" />
                  <div className="truncate">素材包归档</div>
                </div>
              </button>
              <div aria-hidden="true" className="shrink-0 h-7 w-10" />
            </div>
            ...
            <div
              className={`${entryRowBase} ${activeMode === "my" && activeArchiveTab === "roles" ? entryRowActive : entryRowInactive}`}
            >
              <button
                type="button"
                className="flex-1 min-w-0 text-left"
                onClick={() => handleNavigate("/chat/discover/my/roles")}
                aria-label="进入人物归档"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className="size-2 rounded-full bg-primary opacity-70" />
                  <div className="truncate">人物归档</div>
                </div>
              </button>
              <div aria-hidden="true" className="shrink-0 h-7 w-10" />
            </div>
