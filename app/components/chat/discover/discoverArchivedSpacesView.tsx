...
function isRootRepository(repository: Repository) {
  ...
  if (isValidId(rootId)) return rootId === id;
...
function toRepositories(
  result?: ApiResultPageBaseRespRepository,
): Repository[] {
...
export default function DiscoverArchivedSpacesView({
  mode,
}: DiscoverArchivedSpacesViewProps) {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const [keyword, setKeyword] = useState("");
  const [isRepositoryViewModeOpen, setIsRepositoryViewModeOpen] =
    useState(false);
  const activeRepositoryId = useMemo(
    () => parseRepositoryId(searchParams.get("repositoryId")),
    [searchParams],
  );
  ...
  const openRepositoryInPanel = useCallback(
    (repositoryId: number) => {
  ...
  const rootRepositoryQuery = useQuery({
    ...
    queryFn: () =>
      tuanchat.repositoryController.page({
        pageNo: 1,
  ...
  const archivedRepositoryIds = useMemo(() => {
    ...
    for (const space of archivedSpaces) {
      const repoId = space?.repositoryId;
      if (isValidId(repoId)) ids.add(repoId);
    }
  ...
  const archivedRepositoryGroups = useMemo(() => {
    ...
    groups.sort((a, b) => {
      const at =
        toEpochMs(a.latestSpace.updateTime) ||
        toEpochMs(a.latestSpace.createTime);
      const bt =
        toEpochMs(b.latestSpace.updateTime) ||
        toEpochMs(b.latestSpace.createTime);
      return bt - at;
    });
  ...
  const filteredArchivedRepositoryGroups = useMemo(() => {
    const q = normalizeText(keyword);
    if (!q) return archivedRepositoryGroups;

    return archivedRepositoryGroups.filter((group) => {
      const name = normalizeText(
        group.repository?.repositoryName ?? group.latestSpace?.name,
      );
      const desc = normalizeText(
        group.repository?.description ?? group.latestSpace?.description,
      );
      return name.includes(q) || desc.includes(q);
    });
  ...
  const isLoading =
    mode === "square"
      ? rootRepositoryQuery.isLoading
      : archivedSpacesQuery.isLoading;
  const isError =
    mode === "square"
      ? rootRepositoryQuery.isError
      : archivedSpacesQuery.isError;
  const refetch =
    mode === "square"
      ? rootRepositoryQuery.refetch
      : archivedSpacesQuery.refetch;
  ...
  const shouldHideRepositoryHeader = Boolean(
    activeRepositoryId && isRepositoryViewModeOpen,
  );
          ...
          {activeRepositoryId ? (
            ...
            <>
              ...
              {!isLoading &&
                !isError &&
                mode === "square" &&
                filteredRootRepositories.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                    {filteredRootRepositories.map((repository) => {
                      const repositoryId = repository?.repositoryId ?? -1;
                      const name =
                        repository?.repositoryName ?? `仓库 #${repositoryId}`;
                      const description = String(
                        repository?.description ?? "",
                      ).trim();
                      const image =
                        repository?.image ?? DEFAULT_REPOSITORY_IMAGE;
              ...
              {!isLoading &&
                !isError &&
                mode === "my" &&
                filteredArchivedRepositoryGroups.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                    {filteredArchivedRepositoryGroups.map((group) => {
                      ...
                      const name =
                        ...
                        group.latestSpace?.name ??
                        `仓库 #${repositoryId}`;
                      const description = String(
                        repository?.description ??
                          group.latestSpace?.description ??
                          "",
                      ).trim();
                      const image =
                        repository?.image ??
                        group.latestSpace?.avatar ??
                        DEFAULT_REPOSITORY_IMAGE;
